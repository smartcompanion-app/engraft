...
def test_ecmascript_named_group_accepted(work_dir, values_dir):
    ts_file = work_dir / "colors.ts"
    ts_file.write_text('export const PRIMARY_COLOR = "#ff0000";\n')
    ...
    action = RegexReplace(
        ...
        replace=[
            {
                "selector": r'(PRIMARY_COLOR\s*=\s*)"(?<value>[^"]*)"',
                "variable": "primary_color",
            },
        ],
    )
...
def test_normalization_preserves_other_pattern_content(work_dir, values_dir):
    """The (?<value> → (?P<value> rewrite must only affect the value group."""
    ts_file = work_dir / "data.ts"
    ts_file.write_text('KEY = "old"; NOTE = "unchanged";\n')

    # Unnamed groups and literal '<' characters must not be disturbed.
    action = RegexReplace(
        file="data.ts",
        replace=[
            {
                "selector": r'(KEY\s*=\s*)"(?<value>[^"]*)"(\s*;)',
                "variable": "val",
            },
        ],
    )
    action.apply({"val": "new"}, work_dir, values_dir)

    content = ts_file.read_text()
    assert 'KEY = "new";' in content
    assert 'NOTE = "unchanged";' in content


def test_error_mentions_both_syntaxes(work_dir, values_dir):
    ts_file = work_dir / "test.ts"
    ts_file.write_text("hello")
    ...
    action = RegexReplace(
        file="test.ts",
        replace=[
            {"selector": r"(?P<other>hello)", "variable": "val"},
        ],
    )
