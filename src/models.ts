...
const loadYaml = (path: string): unknown => {
  ...
  // `load` rejects an input holding no document at all, but an empty template
  // or values file is a legitimate state — nothing declared, nothing to do.
  // `loadAll` returns an empty list for that, and lets a multi-document file
  // fail with a message that says what is actually wrong.
  const documents = loadAll(text);
  if (documents.length > 1) {
    throw new Error(`${path} must contain a single YAML document, found ${documents.length}`);
  }
  return documents[0];
};
