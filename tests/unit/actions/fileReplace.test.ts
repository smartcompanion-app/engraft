...
describe("FileReplace", () => {
  it("copies source file over target file", async () => {
    ...
    writeFileSync(path.join(root, "work-target"), "target-content");
    // setup dirs
    ...
    await action.apply({ logo: "new-logo.png" }, workDir, valuesDir);
  ...
  it("errors when source file does not exist", async () => {
    ...
    const action = new FileReplace({ file: "logo.png", variable: "logo" });
    await expect(action.apply({ logo: "missing.png" }, workDir, valuesDir)).rejects.toThrow(
  ...
  it("is a noop when the variable is null", async () => {
    ...
    expect(readFileSync(path.join(workDir, "logo.png"), "utf8")).toBe("ORIGINAL");
  });
