...
describe("HtmlReplace", () => {
  ...
  it("replaces an attribute value", async () => {
    ...
    const action = new HtmlReplace({
      file: "index.html",
      replace: [{ selector: '//meta[@name="description"]/@content', variable: "d" }],
    });
  ...
  it("errors on 0 matches", async () => {
    ...
    await expect(action.apply({ t: "x" }, workDir, workDir)).rejects.toThrow(/matched no elements/);
  });
  ...
  it("errors on multiple matches", async () => {
    ...
    await expect(action.apply({ t: "x" }, workDir, workDir)).rejects.toThrow(/matched 2 elements/);
  });
