...
describe("JsonReplace", () => {
  ...
  it("handles nested paths with array indices", async () => {
    const workDir = makeWorkdir({
      "config.json":
        JSON.stringify({ items: [{ label: "a" }, { label: "b" }, { label: "c" }] }, null, 2) + "\n",
    });
    const action = new JsonReplace({
      file: "config.json",
  ...
  it("applies multiple replacements in order", async () => {
    const workDir = makeWorkdir({
      "pkg.json": JSON.stringify({ name: "x", version: "1.0.0" }, null, 2) + "\n",
    });
    const action = new JsonReplace({
      file: "pkg.json",
      replace: [
  ...
  it("skips an entry when its variable is null (optional unset)", async () => {
    const workDir = makeWorkdir({
      "config.json": JSON.stringify({ a: "keep", b: "replace" }, null, 2) + "\n",
    });
    const action = new JsonReplace({
      file: "config.json",
      replace: [
