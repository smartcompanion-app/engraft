...
describe("engine.apply", () => {
  ...
  it("modifies project files on success", async () => {
    ...
    const data = JSON.parse(readFileSync(path.join(fx.projectDir, "config.json"), "utf8"));
  ...
  it("uses default when values omit the override", async () => {
    ...
    const data = JSON.parse(readFileSync(path.join(fx2.projectDir, "config.json"), "utf8"));
    ...
    const fx2 = makeFixture(
      {
        ...
        customizations: [
          ...
          },
          {
            ...
            file: "missing.txt",
            replace: [{ selector: '"(?P<value>x)"', variable: "name" }],
          },
        ],
      },
      {},
